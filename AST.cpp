...
namespace {
	...
	public:
		AssignmentExpr(std::string_view str, Node_t lhs, Node_t rhs)
			: Expression(Kind::ASSIGNMENT, str), lhs_(std::move(lhs)), rhs_(std::move(rhs)) {}
		static bool classof(const Expression *expr) { return expr->getKind() == Kind::ASSIGNMENT; }
		Value Eval() override;
	private:
		Node_t lhs_;
		Node_t rhs_;
	...
	public:
		...
		Value Eval() override;
		static bool classof(const Expression *expr) { return expr->getKind() == Kind::VARIABLE; }
	};
	...
	public:
		IndexExpr(std::string_view str,  Node_t&& var, Node_t&& index) :
			Expression(Kind::INDEX, str), variable_(std::move(var)), index_(std::move(index)) {}
		//IndexExpr() : Expression(Kind::INDEX) {}
		static bool classof(const Expression *expr) { return expr->getKind() == Kind::INDEX; }
		Value Eval() override;
	private:
		Node_t variable_;
		Node_t index_;
	...
	public:
		...
		static bool classof(const Expression *expr) { return expr->getKind() == Kind::STRING_LIT; }
		Value Eval() override;
	private:
	...
	public:
		IntegerLiteral(std::string_view str, std::int64_t value) : Expression(Kind::INT_LIT, str), value_(value) {}
		static bool classof(const Expression *expr) { return expr->getKind() == Kind::INT_LIT; }
		Value Eval() override;
	private:
		std::int64_t value_;
	};
	...
	public:
		ParseBuffer(BufferType  buffer) : buff_(buffer), cur_(buff_.cbegin()), end_(buff_.cend()) {}
...
static Node_t ParseIndexExpression(ParseBuffer &buff, Node_t lhs) {
	...
	buff.SkipWS();
	if (buff.IsEOF()) { throw ParseError("Unexpected end of file"); }
	assert(*buff.Pos() == ']');
	++buff.Pos();
	return std::make_unique<IndexExpr>("dummy"sv, std::move(lhs), std::move(expr));
}
...
static Node_t ParseAssignementExpression(ParseBuffer &buff, Node_t lhs) {
	...
	buff.SkipWS();
	using namespace std;
	return std::make_unique<AssignmentExpr>("assignDummy"sv, std::move(lhs), ParseExpression(buff));

}

// PArse a single statement ((at the moment that means only 1 AssignmentExpr which must be at the top level
...
Value IntegerLiteral::Eval() {
	return value_;
}

Value StringLiteral::Eval() {
	return StringObject::Allocate(value_);
}
...
static Value AssignIndex(IndexExpr* indx, Value value) {
	return 1;
}
...
void TestParser() {
	using namespace std;
	//std::cout << "Hello\n";
	ParseBuffer buff("1234 "sv);
	//ParseIdentifier(buff);
	//std::cout << '"' << ParseIdentifier(buff) << '"' << std::endl;
	ParseIntegerLiteral(buff);
