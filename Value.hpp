...
public:
	...
	constexpr bool IsNill() const { return kind_ == Kind::NILL; /*TODO also maybe check if we are an object which is nill*/ }
...
constexpr bool Value::operator==(const Value& other) const {
	...
	switch (kind_) {
	case Kind::NILL: return true;
	case Kind::INTEGER: return i_ == other.i_;
	case Kind::OBJECT: {
		if (object_ == other.object_) 
			return true;
		if (object_ != nullptr && other.object_ != nullptr) {
			if ((object_->kind() == Object::Kind::STRING) && (other.object_->kind() == Object::Kind::STRING)) {
